...
TEST(DataGenerateTest, BasicTest) {
    ...
    DataConfig test_config = {
        ...
        /*seed=*/42,
        /*distribution=*/"normal",
        /*output_dir=*/"/data/xjs/random_dataset/1024dim100M"
    };
    
    ExperimentGenerator exp_gen(test_config, DistanceMetric::L2);
    
    // 生成并验证
    exp_gen.generate_experiment("1024dim100M", true);
    
    // 加载并展示部分数据
    RandomDataGenerator data_gen(test_config);
    auto database = data_gen.load_vectors("/data/xjs/random_dataset/1024dim100M/base.fbin");
