...
struct bulk_gpuANN {
  ...
  __host__ void process_batch(vertex_data_type &n_vectors,
    ...
    gallatin::utils::timer search_and_prune_timer;
    #endif

    gpu_error::static_timer<0>::start("Total batch time");
    gpu_error::cuda_timer timer;
    timer.start();
    beam_search_and_prune(n_vectors_to_process, nodes_explored_per_iteration,
                          alpha);
    timer.stop();
    //std::cout << "batch_size=" << n_vectors_to_process << " process_time=" << timer.total_time() << std::endl;

    gpu_error::static_timer<0>::stop();

    #if MEASURE_BATCH_TIME
    search_and_prune_timer.sync_end();
    search_and_prune_timer.print_throughput("Inserted", n_vectors_to_process);
    #endif
  ...
  __host__ void construction_round(vertex_data_type ext_n_vertices,
    ...
    while (count < ext_n_vertices) {
      ...
      if (current_batch_size > 0) {
        process_batch(n_vectors_left, nodes_explored_per_iteration, alpha);
      }
