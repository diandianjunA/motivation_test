...
// Generate random uint8 vectors for demonstration
std::vector<uint8_t> generate_random_vectors(uint64_t n, uint32_t dim) {
  std::vector<uint8_t> data(n * dim);
  for (auto &v : data) {
    v = static_cast<uint8_t>(rand() % 256);
  }
  return data;
}
...
int main() {
  ...
  jasper::JasperIndex<DIM> loaded_index;
