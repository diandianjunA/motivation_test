...
MinorCoroutine read_entry_point_probe(RemotePtr& ep_ptr, s_ptr<Node>& node, const u_ptr<ComputeThread>& thread) {
  ep_ptr = co_await rdma::read_entry_point_ptr(thread);
  if (!ep_ptr.is_null()) {
    node = co_await rdma::read_node(ep_ptr, thread);
  }
}
...
ComputeService<Distance>::ComputeService(const Configuration& config, bool shutdown_remote_on_stop)
  ...
  const u32 seed = config_.seed == -1 ? std::random_device{}() : config_.seed + cm_.client_id;
  hnsw_ = std::make_unique<hnsw::HNSW<Distance>>(
    config_.m, config_.ef_construction, config_.k, config_.ef_search, seed, config_.dim, config_.use_cache);

  const size_t estimated_index_size = hnsw_->estimate_index_size(config_.max_vectors);
