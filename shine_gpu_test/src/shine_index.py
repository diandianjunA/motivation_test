...
class ShineVectorIndex(VectorIndex):
    ...
    def __init__(self, host: str = "localhost", port: int = 8080,
                 timeout: int = 3000, wait_for_server: bool = True):
        """
        ...
        Args:
            host: Shine server host
            port: Shine server port
            timeout: Request timeout in seconds (default 3000s for large vectors)
        ...
        def insert_batch(offset):
            ...
            with lock:
                ...
                if completed[0] % 1000 < batch_size:
                    print(f"  Built {completed[0]}/{total} vectors", flush=True)
            return cnt

        print(f"Building index with {total} vectors using {num_threads} threads (batch_size={batch_size})...", flush=True)
    ...
    def build_from_file(self, dataset_path: str, num_threads: int = 4) -> None:
        ...
        print(f"Reading data from {dataset_path}...", flush=True)
        data, info = read_fbin(dataset_path)
        num_vectors, dim = info
        
        print(f"Building index with {num_vectors} vectors of dimension {dim}...", flush=True)
        
        ids = np.arange(num_vectors, dtype=np.uint32)
        self.build(data, ids, num_threads)
        
        print(f"Index built with {num_vectors} vectors", flush=True)
