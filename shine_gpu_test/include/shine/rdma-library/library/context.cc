...
QP Context::connect_to_server(const str& address, u32 tcp_port, u32 node_id) {
  ...
  const int resolve_status = getaddrinfo(address.c_str(), nullptr, &hints, &result);
  lib_assert(resolve_status == 0 && result != nullptr,
             "Cannot resolve server address " + address + ": " + gai_strerror(resolve_status));
  remote_address.sin_addr = reinterpret_cast<sockaddr_in*>(result->ai_addr)->sin_addr;
  freeaddrinfo(result);
