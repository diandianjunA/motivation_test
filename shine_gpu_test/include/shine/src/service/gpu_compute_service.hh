...
#include "gpu/service_scheduler.hh"
...
public:
  explicit GpuComputeService(const Configuration& config, bool shutdown_remote_on_stop = false);
  ~GpuComputeService();

  GpuComputeService(const GpuComputeService&) = delete;
  GpuComputeService& operator=(const GpuComputeService&) = delete;
...
private:
  static size_t estimate_cache_size(const Configuration& config);
...
private:
  ...
  std::unique_ptr<gpu::GpuIndex> index_;
