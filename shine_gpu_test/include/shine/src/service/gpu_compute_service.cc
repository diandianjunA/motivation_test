...
constexpr u32 kRpcMagic = 0x53475055;  // "SGPU"
constexpr u32 kRpcVersion = 1;
constexpr u32 kInitialRpcRecvsPerPeer = 8;
constexpr u32 kMaxRpcResults = 512;

float l2_distance_squared(const vec<element_t>& lhs, const vec<element_t>& rhs) {
...
size_t GpuComputeService::estimate_cache_size(const Configuration& config) {
  const u32 pad_dim = gpu::choose_pad_dim(config.dim);
  const size_t estimated_index_size = gpu::estimate_record_bytes(pad_dim) * static_cast<size_t>(config.max_vectors);
  return static_cast<f32>(estimated_index_size) / 100.0f * config.cache_size_ratio;
}

GpuComputeService::GpuComputeService(const Configuration& config, bool shutdown_remote_on_stop)
    : config_(config),
      context_(config_),
      cm_(context_, config_),
      num_servers_(config_.num_server_nodes()),
      shutdown_remote_on_stop_(shutdown_remote_on_stop),
      cache_size_(estimate_cache_size(config_)) {
  if (config_.num_threads < 2) {
    throw std::runtime_error("GPU compute service requires at least 2 worker threads");
  }
  ...
  worker_pool_->allocate_worker_threads(context_, cm_, remote_access_tokens_, 1);
  cm_.synchronize();

  wait_for_load_or_store();

  index_ = std::make_unique<gpu::GpuIndex>(config_, worker_pool_->get_buffer_allocator(), num_servers_, cache_size_);
  index_->initialize_or_bootstrap(compute_threads()[0], cm_.is_initiator);
...
GpuComputeService::~GpuComputeService() {
  stop_rpc();
  stop_workers();
  shutdown_remote_if_requested();
}

size_t GpuComputeService::insert(const vec<InsertItem>& batch) {
...
vec<node_t> GpuComputeService::search_local(const vec<element_t>& query, u32 k) {
...
vec<node_t> GpuComputeService::search(const vec<element_t>& query, u32 k) {
  if (!routing_enabled()) {
...
bool GpuComputeService::load_index(const std::string& path, str* error_message) {
  pause_workers();
  pause_rpc();
  try {
    ...
    index_->clear_cache();
    index_->reload_from_remote(compute_threads()[0]);
    resume_rpc();
    ...
    if (error_message) {
      *error_message = "unknown load error";
    }
    resume_rpc();
    resume_workers();
    return false;
...
bool GpuComputeService::store_index(const std::string& path, str* error_message) {
  pause_workers();
  pause_rpc();
...
GpuComputeService::Status GpuComputeService::status() const {
  return {
    .state = "running",
    .vectors_inserted = vectors_inserted_.load(std::memory_order_relaxed),
    .dimension = index_->dim(),
    .threads = config_.num_threads,
    .indexed_vectors = index_->size(),
  };
}

bool GpuComputeService::routing_enabled() const {
  return config_.routing && cm_.num_total_clients > 1;
}

size_t GpuComputeService::rpc_message_size() const {
...
vec<element_t> GpuComputeService::compute_local_routing_centroid() const {
  return index_->routing_centroid();
}

void GpuComputeService::init_remote_tokens() {
...
void GpuComputeService::receive_remote_access_tokens() {
...
void GpuComputeService::wait_for_load_or_store() {
  ...
  for (idx_t i = 0; i < num_memory_servers; ++i) {
    ...
    if (cmd != mn_command::NOOP) {
      path = gpu_index_path::shard_file(index_prefix, i + 1, num_memory_servers).string();
    }
  ...
  for (idx_t i = 0; i < num_memory_servers; ++i) {
    ...
    if (resp.message_length > 0) {
      message.resize(resp.message_length);
      LocalMemoryRegion msg_region{context_, message.data(), resp.message_length};
      cm_.server_qps[i]->post_receive(msg_region);
      context_.receive();
    }

    const str detail = message.empty() ? "" : ": " + message;
    lib_assert(resp.success, "startup load/store failed on memory server " + std::to_string(i) + detail);
  }
}

vec<GpuComputeService::CommandResult> GpuComputeService::send_index_command(mn_command::Command cmd,
                                                                            const std::string& path) {
  std::lock_guard<std::mutex> lock(mn_command_mutex_);
  vec<CommandResult> results;
  results.reserve(cm_.server_qps.size());

  for (idx_t i = 0; i < cm_.server_qps.size(); ++i) {
    std::string node_path = path;
    if (!path.empty()) {
      node_path = gpu_index_path::shard_file(filepath_t(path), i + 1, cm_.server_qps.size()).string();
    }

    mn_command::Request req{cmd, node_path.size()};
    QP& qp = cm_.server_qps[i];
    qp->post_send_inlined(&req, sizeof(mn_command::Request), IBV_WR_SEND);
    context_.poll_send_cq_until_completion();
  ...
  for (idx_t i = 0; i < cm_.server_qps.size(); ++i) {
    ...
    results.push_back({resp.success, std::move(message)});
  }

  return results;
}

void GpuComputeService::start_workers() {
  const u32 batch_size = std::max<u32>(1, config_.num_coroutines);
  print_status("starting " + std::to_string(config_.num_threads) + " GPU service worker threads");

  workers_.reserve(config_.num_threads);
  workers_.emplace_back([this, batch_size]() {
    service::gpu_schedule_inserts(
      *index_, insert_queue_, shutdown_, batch_size, compute_threads()[0], workers_paused_, workers_idle_count_);
  });

  for (u32 tid = 1; tid < config_.num_threads; ++tid) {
    workers_.emplace_back([this, batch_size, tid]() {
      service::gpu_schedule_queries(
        *index_, query_queue_, shutdown_, batch_size, compute_threads()[tid], workers_paused_, workers_idle_count_);
    });
  }
...
void GpuComputeService::stop_workers() {
...
void GpuComputeService::pause_workers() {
...
void GpuComputeService::resume_workers() {
  workers_paused_.store(false, std::memory_order_release);
}

void GpuComputeService::start_rpc() {
  if (!routing_enabled()) {
    return;
  }
...
void GpuComputeService::stop_rpc() {
  if (!routing_enabled()) {
    return;
  }
...
void GpuComputeService::pause_rpc() {
  if (!routing_enabled()) {
    return;
  }
...
void GpuComputeService::resume_rpc() {
  rpc_paused_.store(false, std::memory_order_release);
}

void GpuComputeService::post_rpc_receive(u32 peer_client) {
...
void GpuComputeService::post_initial_rpc_receives() {
  if (cm_.is_initiator) {
...
QueuePair& GpuComputeService::qp_for_client(u32 client_id) {
  if (cm_.is_initiator) {
...
void GpuComputeService::enqueue_rpc(RpcOutbound* outbound) {
  outbound_rpc_queue_.enqueue(outbound);
}

void GpuComputeService::flush_outbound_rpc() {
...
u32 GpuComputeService::choose_destination(const vec<element_t>& query) const {
  ...
  for (u32 client = 0; client < routing_centroids_.size(); ++client) {
    ...
    const float distance = l2_distance_squared(query, routing_centroids_[client]);
...
void GpuComputeService::handle_register_centroid(const RpcHeader& header, const byte_t* payload) {
  if (!cm_.is_initiator) {
    return;
  }
...
void GpuComputeService::handle_register_ack(const RpcHeader& header) {
...
void GpuComputeService::handle_search_proxy(const RpcHeader& header, const byte_t* payload) {
  if (!cm_.is_initiator) {
    return;
  }
...
void GpuComputeService::handle_search_request(const RpcHeader& header, const byte_t* payload) {
  vec<element_t> query(header.payload_count, 0.0f);
  if (!query.empty()) {
    std::memcpy(query.data(), payload, query.size() * sizeof(element_t));
  }
...
void GpuComputeService::handle_search_response(const RpcHeader& header, const byte_t* payload) {
...
void GpuComputeService::handle_rpc_receive(const RpcHeader& header, const byte_t* payload) {
...
void GpuComputeService::run_rpc_loop() {
  if (!routing_enabled()) {
    return;
  }
...
void GpuComputeService::refresh_routing_state(bool wait_for_remote_registration) {
  if (!routing_enabled()) {
    return;
  }
...
void GpuComputeService::shutdown_remote_if_requested() {
